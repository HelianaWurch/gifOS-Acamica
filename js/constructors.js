...
const noFavsImg = createImgElement(
  'no-favs-img',
  './assets/icon-fav-sin-contenido.svg',
  'no favs'
);
const noFavsParagraph = createElement('p', 'no-p no-favs-p');
noFavsParagraph.innerHTML =
...
const visibleOpacityElement = (element) => {
  element.classList.contains('ceroopacity')
    ? element.classList.remove('ceroopacity') &&
      element.classList.toggle('fullopacity')
    : element.classList.toggle('ceroopacity') &&
...
function gifConstructor(src, title, user, id, srcid, container) {
  ...
  const gifButtonMax = createElement(
    'button',
    'card-btn max-btn',
    `gif-max-${user + id}`
  );
...
function saveMyFavMap() {
  localStorage.setItem(
    'favoritos',
    JSON.stringify(Array.from(myFavoritesGifsArray.entries()))
  );
}
